...
def hello_world():
    ...
    if request_json['sessionUser'] in access_list:
        ...
        if input_list.count(None) == len_of_calls:
            ...
            try:
                syslog.syslog("HIT")
                output_str = reidentify.reidentify_deterministic(input_str = input_str,key_name = key_name)
                ...
                if "400 Too many findings to de-identify".lower() in str(e).lower():
                    ...
                    try:
                        ...
                        for count, new_batch in enumerate(new_batches,start=1): 
                            ...
                            output_str = reidentify.reidentify_deterministic(input_str = input_str_new,key_name = key_name)
                        ...
                        if "429 Quota exceeded".lower() in str(e).lower():
                            syslog.syslog("Encountered 429 error, sending it back to BQ with error code as 429")
                            return flask.jsonify({"errorMessage": "429 Quota limit error"}), 429
                        else:
                            syslog.syslog("Got new error after breaking: "+str(e))
                            return flask.jsonify({"errorMessage": "New error encountered, hence stopping the process"}), 500
                    ...
                    return flask.jsonify({"errorMessage": "New error encountered, hence stopping the process"}), 400
