...
def employee_competencies_gen_start_function():
    ...
    bucket_name =get_bucket_name(project_name,config_file.bucket_prefix)
    ...
    output_file_name=config_file.destination_file_name_prefix+str(datetime.now().strftime('%Y%m%d%H%M'))+".json"
    ...
    write_as_json(bucket_name,output_file_with_bucket, json_list)
