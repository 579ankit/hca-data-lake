...
bucket_names = 'hca_hospital-reports_source_20231005'

date_pattern = r'\d{12}'
folder_prefixes = ['lab_report_object_data/'] 
...
def move_and_rename_bucket(bucket_names):
    for source_bucket_name in bucket_names:
        ...
        for each_file in file_list:
            ...
            file_name = each_file_name.replace('lab_report_object_data/', '')
            destination_blob_name = f"lab_report_object_data/{date}/{file_name}"
...
dag = DAG(
    'move_unstructured_data',
    default_args=default_args,
    description='DAG to move unstructured files in GCS buckets',
    schedule_interval=None,
)

move_unstructured_data_task = PythonOperator(
    task_id='move_unstructured_data',
